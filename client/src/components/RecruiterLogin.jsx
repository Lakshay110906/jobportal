...
const RecruiterLogin = () => {
    ...
    const [isTextDataSumbited, setIsTextDataSumbited] = useState(false)
    ...
    const onSumbitHandler = async (e) => {
        ...
        if (state === 'Sign Up' && !isTextDataSumbited) {
            return setIsTextDataSumbited(true)
        }
    ...
    return (
        <div className='absolute top-0 left-0 bottom-0 right-0 z-10 backdrop-blur-sm bg-black/30 flex justify-center items-center '>
            <form onSubmit={onSumbitHandler} className='relative bg-white p-10 rounded-xl text-slate-500'>
                <h1 className='text-center text-2xl text-neutral-700 font-medium '>Recruiter {state}</h1>
                <p className='text-sm'>Welcome back! Please sign in to continue</p>
                {
                    state === 'Sign Up' && isTextDataSumbited ? <>
                        ...
                        <>
                            ...
                            {state !== 'Login' && (
                                <div className='border px-4 py-2 flex items-center gap-2 rounded-full mt-5'>
                                    ...
                                    <input className='outline-none text-sm' type="text" placeholder='Comapny Name' required onChange={e => setName(e.target.value)} value={name} />
                                </div>
                            )}
                            <div className='border px-4 py-2 flex items-center gap-2 rounded-full mt-5'>
                ...
                <button type='submit' className='bg-blue-600 mt-4  w-full text-white py-2 rounded-full'>
                    {state === 'Login' ? 'login' : isTextDataSumbited ? 'create account' : 'next'}
                </button>
